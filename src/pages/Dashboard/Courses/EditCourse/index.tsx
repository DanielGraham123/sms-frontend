...
const index = () => {
    ...
    const saveCourse = (e: React.MouseEvent) => {
        ...
        CourseService.updateCourse(courseId!, course_).then((response) => {
            setLoading(true)
            setTimeout(() => {
    ...
    const fetchCourse = async (id: number) => {
        CourseService.getCourseById(id).then((response) => {
            console.log("Course: ", response);
            setCourse(response)
            setEditorData(response.description)
            setSelectedProgramme(response.programme.id.toString())
            setSelectedClasses(response.grades.map((grade: GradeU) => grade.id.toString()))
        }).catch((res) => {
            console.log("Error: ", res);
        })
    }
